...
def add_book_window():
    ...
    tk.Label(add_win, text="Book ID:").pack()
    ...
    tk.Button(add_win, text="Generate New ID", 
             command=lambda: id_entry.config(state='normal') or id_entry.delete(0, tk.END) or 
             id_entry.insert(0, generate_book_id()) or id_entry.config(state='readonly')).pack(pady=5)
