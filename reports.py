...
def open_report_window():
    ...
    Radiobutton(window, text="Overdue Only", variable=report_type, value="overdue", bg="#f2f2f2").pack(anchor=W)

    def on_generate():
        rt = report_type.get()
        if rt == "full":
            generate_report("All", "All", False)
        elif rt == "overdue":
            generate_report("All", "All", True)
            ...
            overdue_var_popup = BooleanVar(value=False)
            Checkbutton(popup, text="Overdue Only", variable=overdue_var_popup).grid(row=2, columnspan=2, pady=5)

            def confirm():
                generate_report(grade_var.get(), stream_var.get(), overdue_var_popup.get())
                popup.destroy()

            Button(popup, text="Generate", command=confirm, bg="green", fg="white").grid(row=3, columnspan=2, pady=10)
        elif rt == "period":
            popup = Toplevel(window)
            ...
            def confirm():
                try:
                    ...
                    generate_report("All", "All", overdue_var_popup.get(), start, end)
                    popup.destroy()
                except ValueError:
